...
# 3. rewrite the history on that branch to *only* include staging/src/k8s.io/sample-apiserver
...
# this command rewrite git history to *only* include staging/src/k8s.io/sample-apiserver
git filter-branch -f --subdirectory-filter staging/src/k8s.io/sample-apiserver HEAD
